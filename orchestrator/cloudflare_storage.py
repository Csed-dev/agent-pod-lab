    ...
    def __init__(self):
        if not CF_ACCOUNT_ID or not CF_API_TOKEN or not CF_D1_DATABASE_ID:
            raise EnvironmentError(
                ...
                "  CF_API_TOKEN — API token with D1 Read/Write permission\n"
...
class R2Client:
    def __init__(self):
        if not CF_ACCOUNT_ID or not CF_R2_ACCESS_KEY or not CF_R2_SECRET_KEY:
            raise EnvironmentError(
    ...
    def download_bytes(self, remote_key: str) -> bytes:
        response = self._s3.get_object(Bucket=self._bucket, Key=remote_key)
