...
from lib.architectures.neumann_gelu import (
    PolyMPNN_GELU, PolynomialPreconditioner, poly_frobenius_loss,
...
model = PolyMPNN_GELU(NUM_LAYERS, EMBED_DIM, HIDDEN_DIM, 2, POLY_DEGREE).to(device)
num_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
print(f"Model: PolyMPNN_GELU ({num_params:,} params)")
