...
def compute_eigenmetrics(A, omega):
    ...
    lmin_abs = float(np.min(np.abs(eigenvalues)))
    cond = lmax / lmin_abs if lmin_abs > 1e-15 else float("inf")

    D_inv = sparse.diags(1.0 / A.diagonal())
    J = sparse.eye(A.shape[0]) - omega * (D_inv @ A)
    j_eigs = np.linalg.eigvalsh(J.toarray())
    rho = float(np.max(np.abs(j_eigs)))

    pos_def = bool(np.all(eigenvalues > 0))
    diag_dom = True
...
def main():
    ...
    for grid_rows, grid_cols in GRIDS:
        ...
        A_base = build_stencil(grid_rows, grid_cols)
        ...
        for eps in EPSILON_VALUES:
            for seed_idx in range(N_SEEDS):
                ...
                A_noisy = add_noise(A_base, eps, rng)

                has_zero_diag = np.any(np.abs(A_noisy.diagonal()) < 1e-15)
                if has_zero_diag:
                    for solver_name in SOLVERS:
                        all_results.append({
                            "grid": grid_label, "n": n, "epsilon": eps,
                            "seed": seed, "solver": solver_name,
                            ...
                            "diag_dominant": 0.0, "weyl_bound": wb[eps],
                            "empirical_shift": float("nan"),
                        })
                ...
                empirical_shift = max(
                    abs(metrics["lambda_min"] - float(base_eigenvalues[0])),
                    abs(metrics["lambda_max"] - float(base_eigenvalues[-1])),
                )
    ...
    for grid_rows, grid_cols in GRIDS:
        grid_label = f"{grid_rows}x{grid_cols}"
        print(f"\n--- Grid {grid_label} ---")
        print(f"{'eps':>6} | {'solver':>8} | {'iters':>6} | {'time_ms':>8} | {'fail%':>5} | {'rho':>6} | {'kappa':>8} | {'weyl':>6} | {'emp_shift':>9}")
        for eps in EPSILON_VALUES:
            for solver_name in SOLVERS:
                rows = [r for r in all_results
                        if r["grid"] == grid_label and r["epsilon"] == eps and r["solver"] == solver_name]
                if not rows:
                    continue
                ...
                print(f"{eps:6.2f} | {solver_name:>8} | {avg_iters:6.1f} | {avg_time:8.1f} | {fail_rate:5.1f} | {avg_rho:6.3f} | {avg_kappa:8.1f} | {avg_weyl:6.3f} | {avg_shift:9.3f}")

    num_completed = len(all_results)
    print(f"\ntotal_time: {t_total:.1f}")
    print(f"num_completed_configs: {num_completed}")
    print(f"num_grids: {len(GRIDS)}")
    print(f"num_epsilon_values: {len(EPSILON_VALUES)}")
    print(f"num_seeds: {N_SEEDS}")
    print(f"num_solvers: {len(SOLVERS)}")
