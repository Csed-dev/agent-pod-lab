...
from lib.architectures.neumann import (
    PolyMPNN, PolynomialPreconditioner, poly_frobenius_loss,
...
model = PolyMPNN(NUM_LAYERS, EMBED_DIM, HIDDEN_DIM, 2, POLY_DEGREE).to(device)
num_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
print(f"Model: PolyMPNN ({num_params:,} params) + AdamW optimizer")
...
train_result = train_loop(model, dataset, loss_fn, save_checkpoint,
                          TrainConfig(lr=LR, optimizer_class="adamw"))
print(f"\nTraining done: {train_result.num_epochs} epochs, best loss: {train_result.best_loss:.4e}")

eval_model = load_checkpoint(train_result.checkpoint_path, device)
def build_preconditioner(mdl, A):
    mdl.set_matrix(A); coeffs = mdl()
...
print_results(results, num_params, train_result.num_epochs, train_result.training_seconds,
              t_end - t_start, peak_vram_mb, num_domains, train_result.best_loss)
